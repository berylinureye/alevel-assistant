...
flowchart TD
  ...
  S --> R{"是否开启快速首题模式"}
  R -->|"fast_batch=true<br/>要先出首个可信结果"| I["页面级识别<br/>尽量并行"]
  R -->|"fast_batch=false<br/>要完整复核或完整讲解"| G["质量优先识别"]
...
这个判断目前由请求参数 `fast_batch` 决定，不是由设备类型决定。前端在希望用户尽快看到第一道可用批改结果时传 `fast_batch=true`；如果调用方更重视完整复核、完整讲解、多模型路径或保守批改，就传 `fast_batch=false`。

## 详细设计解释

### 1. 上传入口为什么分图片和 PDF

图片和 PDF 的用户意图不同。图片上传通常是“我现在要快点知道这页作业哪里错了”；Large PDF 通常是“我上传了一整套真题，需要先选页”。所以图片路径更适合快速首题返回，PDF 则先做缩略图和选页，避免把封面、空白页、答案页一起送进批改。
...
什么时候要开：用户此刻需要先看到第一道可信批改结果，接受剩余慢题稍后补齐或以 `needs_review` 呈现。

什么时候不要开：任务更看重完整复核、完整讲解、多模型批改、严格顺序输出，或调用方明确希望慢一点但更保守。
